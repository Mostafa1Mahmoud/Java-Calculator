...
import com.GUI_Project.Operations;
import com.GUI_Project.OverflowException;
...
public class DoubleEvaluator extends AbstractEvaluator<Double> {
	...
	/** Returns the decimal logarithm of a number with base 10 */
	public static final Function LOG10 = new Function(("log10"), 1);
	/** Returns a pseudo random number */
	public static final Function RANDOM = new Function("random", 0);
	/** The Square root of operator */
	public static final Function SQRT = new Function("√",1);
	/** The Cubic root of operator */
	public static final Function CURT = new Function("∛",1);
	/** The Exponent of operator */
	public static final Function EXPONEN = new Function("e",1);
	...
	public static final Operator FACTORIAL = new Operator("!",1, Operator.Associativity.LEFT,2);
	/** The Combinations operator */
	public static final Operator Combine = new Operator("C",2, Operator.Associativity.LEFT,2);
	/** The Permutations operator */
	public static final Operator Permut = new Operator("P",2,Operator.Associativity.LEFT,2);

	/** The standard whole set of predefined operators */
	private static final Operator[] OPERATORS = new Operator[]{NEGATE, MINUS, PLUS, MULTIPLY, DIVIDE, EXPONENT, MODULO,FACTORIAL,Combine,Permut};
	...
	private static final Function[] FUNCTIONS = new Function[]{SINE, COSINE, TANGENT, ASINE, ACOSINE, ATAN, SINEH, COSINEH, TANGENTH, MIN, MAX, SUM, AVERAGE, LN, LOG,LOG10, ROUND, CEIL, FLOOR, ABS, RANDOM,SQRT,CURT,EXPONEN};
	 ...
	 */
	@Override
	protected Double evaluate(Operator operator, Iterator<Double> operands, Object evaluationContext)throws OverflowException {
		if (NEGATE.equals(operator) || NEGATE_HIGH.equals(operator)) {
			...
			Operations factorial = new Operations();
			return factorial.Factorial(operands.next());
		} else if (Combine.equals(operator)) {
			Operations combin = new Operations();
			return combin.Combination(operands.next(), operands.next());
		} else if (Permut.equals(operator)) {
			Operations perm = new Operations();
			return perm.Permutation(operands.next(), operands.next());
		}else {
	...
	protected Double evaluate(Function function, Iterator<Double> arguments, Object evaluationContext) {
		...
		} else if (CURT.equals(function)) {
			result = Math.cbrt(arguments.next());
		}else if(EXPONEN.equals(function)){
			result = Math.exp(arguments.next());
		}else {
